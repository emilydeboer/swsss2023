#!/usr/bin/env python

import numpy as np
import matplotlib.pyplot as plt
from tridiagonal import solve_tridiagonal
from densityarray import build_dens

# ----------------------------------------------------------------------
# Main code
# ----------------------------------------------------------------------

if __name__ == "__main__":

    dx = 4

    # set x with 1 ghost cell on both sides:
    x = 100 + np.arange(-dx, 400 + 2*dx, dx)
    # alt = 100+x*40

    t_lower = 200.0 
    t_upper = 1000.0

    nPts = len(x)

    # set default coefficients for the solver:
    a = np.zeros(nPts) - 1
    b = np.zeros(nPts) + 2
    c = np.zeros(nPts) - 1
    d = np.zeros(nPts)
    
    # additions
    lam = 80
    
    # time dependent term
    nDays = 3
    dt = 1
    times = np.arange(0, nDays*24, dt)
    lon = 0.0
    
    # instantiate figure
    fig = plt.figure(figsize = (10,10))
    ax1 = fig.add_subplot(221)
    ax2 = fig.add_subplot(222)
    ax3 = fig.add_subplot(223)
    ax4 = fig.add_subplot(224)

    # temperature grid to store temps at different times
    temp_grid = np.zeros([len(times), len(x)])
    dens_grid_N2 = np.zeros([len(times), len(x)])
    dens_grid_O2 = np.zeros([len(times), len(x)])
    dens_grid_O = np.zeros([len(times), len(x)])
    O2_m_AMU = 32 
    O2_n0 = 0.3e19
    O_m_AMU = 16
    O_n0 = 1e18
    
    # diurnal and semidurnal
    AmpDi = 10
    AmpSd = 5
    PhaseDi = np.pi/2
    PhaseSd = 3*np.pi/2
    
    # adding wavelengths
    f107 = 100 + 50/(25*365)*times + 25*np.sin(times/(27*24)*2*np.pi)

    for i, hour in enumerate(times):
        ut = hour % 24
        local_time = lon/15 + ut
        
        factor = -np.cos(local_time/24*2*np.pi)
        if factor < 0:
            factor = 0
            
        sun_heat = f107[i]*.4/100
    
        # setting Q for only some altitudes
        cond = np.logical_and(x < 400, x > 200)
        
        QEUV = np.zeros(nPts)    
        QEUV[cond] = -sun_heat*factor
        
        Qback = np.zeros(nPts)
        Qback[cond] = -0.4
       
        Qtotal = Qback + QEUV
        
        # change in altitude and final d column for tridiagonal solver
        dz = x[1] - x[0]
        dz2 = dz*dz
        d[:] = -Qtotal*dz2/lam

        # boundary conditions (bottom - fixed):
        a[0] = 0
        b[0] = 1
        c[0] = 0
        d[0] = t_lower + AmpDi*np.sin(local_time/24*2*np.pi + PhaseDi) + \
                AmpSd*np.sin(local_time/24*2*2*np.pi + PhaseSd)

        # top - fixed:
        a[-1] = 1
        b[-1] = -1
        c[-1] = 0
        d[-1] = 0
    
        # solve for Temperature:
        t = solve_tridiagonal(a, b, c, d)
        temp_grid[i][:] = t # store in grid
        dens_grid_N2[i][:] = build_dens(t, x)
        dens_grid_O2[i][:] = build_dens(t, x, O2_m_AMU, O2_n0)
        dens_grid_O[i][:] = build_dens(t, x, O_m_AMU, O_n0)
        

    # back in main

    
    cs1 = ax1.contourf(times/24, x, np.array(temp_grid.T))
    ax1.set_ylabel('Altitude (km)')
    ax1.set_xlabel('Time (days)')
    ax1.set_title('Temperature')
    cbar = plt.colorbar(cs1, label = 'Temperature (K)')
    
    cs2 = ax2.contourf(times/24, x,  np.array(np.log10(dens_grid_N2).T))
    ax2.set_ylabel('Altitude (km)')
    ax2.set_xlabel('Time (days)')
    ax2.set_title('Density Map of N2')
    cbar = plt.colorbar(cs2, label = 'Density (per cubic meter)')
    
    cs3 = ax3.contourf(times/24, x,  np.array(np.log10(dens_grid_O2).T))
    ax3.set_ylabel('Altitude (km)')
    ax3.set_xlabel('Time (days)')
    ax3.set_title('Density Map of O2')
    cbar = plt.colorbar(cs3, label = 'Density (per cubic meter)')
    
    cs4 = ax4.contourf(times/24, x,  np.array(np.log10(dens_grid_O).T))
    ax4.set_ylabel('Altitude (km)')
    ax4.set_xlabel('Time (days)')
    ax4.set_title('Density Map of O')
    cbar = plt.colorbar(cs4, label = 'Density (per cubic meter)')
    
    plotfile = 'conduction_v1.png'
    print('writing : ',plotfile)    
    fig.savefig(plotfile)
        
    